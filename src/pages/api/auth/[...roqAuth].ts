...
export default RoqAuth({
  hooks: {
    ...
    onRegisterSuccess: async ({ user }) => {
      ...
      if (user.roles.data.some((role) => ['Library Administrator'].includes(role.name))) {
        const tenant = await roqClient.asSuperAdmin().tenant({ id: user.tenantId });
        const { name: tenantName } = tenant.tenant ?? {};
        seedNumber = await prisma.library.count();
        const mainEntity = await prisma.library.create({
          data: {
            tenant_id: user.tenantId,
            user_id: owner.id,
            name: tenantName,

...
import { getIsbns } from 'apiSdk/isbns';
import { IsbnInterface } from 'interfaces/isbn';
import { getBooks } from 'apiSdk/books';
import { BookInterface } from 'interfaces/book';

import { IsbnListPage as ShowCaseList } from 'pages/isbns';
...
function DashboardPage() {
  ...
  const isbnStat = useSWR<PaginatedInterface<IsbnInterface>>(
    () => `/isbns`,
    () => getIsbns({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Isbns',
      result: isbnStat?.data?.totalCount,
      isLoading: isbnStat.isLoading,
    },
    {
      title: 'Books',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`isbns`} passHref legacyBehavior>
                  <Button

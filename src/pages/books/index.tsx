...
export function BookListPage(props: BookListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getBooks({
        relations: ['library', 'isbn', 'borrowing.count', 'reservation.count'],
    ...
    hasAccess('library', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/libraries/view/${record.library?.id}`}>
              {record.library?.name}
            </Link>
          ),
        }
      : null,
    hasAccess('isbn', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'isbn',
          header: 'Isbn',
          accessorKey: 'isbn',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/isbns/view/${record.isbn?.id}`}>
              {record.isbn?.number}
            </Link>
          ),
        }
      : null,
    hasAccess('borrowing', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'borrowing',
          header: 'Borrowing',
          accessorKey: 'borrowing',
          cell: ({ row: { original: record } }: any) => record?._count?.borrowing,
        }
      : null,

...
import { getBorrowingById } from 'apiSdk/borrowings';
import { BorrowingInterface } from 'interfaces/borrowing';

function BorrowingViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<BorrowingInterface>(
    () => (id ? `/borrowings/${id}` : null),
    () =>
      getBorrowingById(id, {
        relations: ['book', 'user'],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('borrowing', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/borrowings/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Borrow Date"
                  text={data?.borrow_date ? format(parseISO(data?.borrow_date as unknown as string), 'dd-MM-yyyy') : ''}
                />

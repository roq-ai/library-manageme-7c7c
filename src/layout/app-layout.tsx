...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['library-administrator', 'librarian'];
  ...
  const { data, error, isLoading } = useSWR<LibraryInterface[]>(
    ...
    () => getLibraries({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    { name: 'Borrowings', path: '/borrowings', entity: 'borrowing', service: AccessServiceEnum.PROJECT, icon: FiClock },
    { name: 'Isbns', path: '/isbns', entity: 'isbn', service: AccessServiceEnum.PROJECT, icon: FiBox },
    {
      name: 'Reservations',
      path: '/reservations',
      entity: 'reservation',
